...
conn_string=f"host=hadoop-04.uni.innopolis.ru port=5432 user=team9 \
dbname=team9_projectdb password={password}"
...
with psql.connect(conn_string) as conn:
    ...
    with open(os.path.join("sql","сreate_tables.sql"), encoding="UTF-8") as file:
        content = file.read()
        cur.execute(content)
    conn.commit()
    ...
    with open(os.path.join("sql", "import_data.sql"), encoding="UTF-8") as file:
        ...
        with open(os.path.join("data","Ad_click_predict.csv"), "r", encoding="UTF-8") as crts:
            cur.copy_expert(commands[0], crts)
