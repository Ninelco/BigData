...
conn_string="host=hadoop-04.uni.innopolis.ru port=5432 user=team9 dbname=team9_projectdb password={}".format(password)
...
with psql.connect(conn_string) as conn:
        ...
        with open(os.path.join("sql","сreate_tables.sql")) as file:
                content = file.read()
                cur.execute(content)
        conn.commit()
        ...
        with open(os.path.join("sql", "import_data.sql")) as file:
                ...
                with open(os.path.join("data","Ad_click_predict.csv"), "r") as crts:
